   ...
   20    continue
      ...
      if (ifcomp) then 
          if(nio.eq.0) write(6,*)'computing stokes solution'
          call compute_vol_soln(vxc,vyc,vzc,prc)
          call outpost(vxc,vyc,vzc,prc,t,'Stk')
      endif
      delta_flow = get_ubar(vxp(:,1),vyp(:,1),vzp(:,1))
      ...
      if (nio.eq.0) write(6,*) ' vol_flow_lin =', delta_flow
      call add2s2(vxp(:,1),vxc,-delta_flow,ntot1)
      call add2s2(vyp(:,1),vyc,-delta_flow,ntot1)
      call add2s2(vzp(:,1),vzc,-delta_flow,ntot1)
      call add2s2(prp(:,1),prc,-delta_flow,ntot2)
      delta_flow = get_ubar(vxp(:,1),vyp(:,1),vzp(:,1))
      if (nio.eq.0) write(6,*) ' vol_flow_lin =', delta_flow

      end subroutine vol_flow_lin
      ...
      subroutine save_ubar
         ...
         if (nid == 0) then
             write(9833, "(2E15.7)") time, current_ubar ! write to file
             write(6, "(A, F10.3, A, F10.2, A, F10.2)") 'ubar=', current_ubar, 
     $       ' Re=', current_ubar/param(2), ' Re(1)=', (1.0/param(2))/current_ubar
         endif
...
!=========================================================================
      ...
      subroutine makebf_str_pulse(ffx_p,ffy_p,ffz_p,ix,iy,iz,ieg)
         ...
         integer :: ix, iy, iz, ieg, e, nbes, ierr
         real :: dpds, dp1ds, dpdt
         real :: ffx_p, ffy_p, ffz_p
         real :: p, pp, alpha, rr, angle_t, delta, tau, fshape
         complex :: sqrtmi, wwi, binv_pulse, j0_b, j1_b, jn_b(2)
         complex, parameter :: imag = (0.0d0, 1.0d0)

         ! Constants and Pre-computation
         sqrtmi = (1.0d0 - imag) / sqrt(2.0d0)
         e = gllel(ieg)
         
         ! Compute angle_t and pp once
         pp = xax(ix,iy,iz,e)**2 + yax(ix,iy,iz,e)**2
         angle_t = atan2(xax(ix,iy,iz,e), yax(ix,iy,iz,e))

         ! Compute p and related values
         p = sqrt(pp) - curv_radius
         alpha = atan2(p, zax(ix, iy, iz, e))
         rr = p**2 + zax(ix, iy, iz, e)**2
         ...
         wwi = sqrtmi * womersley

         ! Call Bessel function and compute binv_pulse
         call cbesj(wwi, 0.0d0, 1, 2, jn_b, nbes, ierr)
...
!=========================================================================
      ...
      subroutine makebf_str_pulse_old(ffx_p,ffy_p,ffz_p,ix,iy,iz,ieg)
         implicit none
         ...
         parameter (sqrtmi=(1.0d0-imag)/sqrt(2.0d0))
         complex binv_pulse
