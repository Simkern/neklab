      module neklab_2Dh_axisym
         ...
         public :: nek_advance_2Dh_ifaxis
      
      contains

         subroutine nek_advance_2Dh_ifaxis(alpha)
            implicit none
            ...
            character(len=*), parameter :: this_procedure = 'nek_advance_ifaxis'
            ...
            if (.not. torus_coeffs_defined .or. alpha /= alpha_cached) &
               call build_torus_coeffs(alpha)
            ...
            do jp = 1, npert
               ...
               call compute_gradp_ifaxis(gradp, prextr, alphaR_coef)
            ...
            if (alpha /= 0.0_dp .and. ifvcor) then
               ...
               call pressure_matvec_2Dh_ifaxis(ep, onep, h1, h2, h2inv, h2z_shift, intype)
               ebar = glsum(ep, ntot2)
            end if
            ...
            do jp = 1, npert
               ...
               call compute_frc_div_ifaxis(frc_div, tp, alphaR_coef)
               call add2 (dpr(1,jp), frc_div(1,1,1,1,jp), ntot2)
               call chsign(dpr(1,jp), ntot2)

               if (alpha == 0.0_dp) call ortho(dpr(1,jp))

               if (ifprjp) call setrhs_pressure_2Dh_ifaxis(dpr(1,jp), h1, h2, h2inv, pbasis(1,1,jp), nprev(jp), alphaR_coef, info_str)
               call               solve_pressure_2Dh_ifaxis(dpr(1,jp), h1, h2, h2inv, alphaR_coef, alpha, intype, iter, ebar, info_str)
               if (ifprjp) call gensoln_pressure_2Dh_ifaxis(dpr(1,jp), h1, h2, h2inv, pbasis(1,1,jp), nprev(jp), alphaR_coef)
            end do
            ...
            do jp = 1, npert
               ...
               call compute_dw_ifaxis(dw, dpr, h2inv, alphaR_coef)
         ...
         end subroutine nek_advance_2Dh_ifaxis
         ...
         subroutine compute_gradp_ifaxis(gradp, prextr, alphaR)
            implicit none
            include 'SIZE'
            include 'SOLN'
            include 'MASS'
         ...
         end subroutine compute_gradp_ifaxis
         
         subroutine compute_dw_ifaxis(dw, dpr, h2inv, alphaR)
            implicit none
            include 'SIZE'
            include 'SOLN'
            include 'MASS'
         ...
         end subroutine compute_dw_ifaxis
         
         subroutine compute_frc_div_ifaxis(frc_div, w, alphaR)
            implicit none
            include 'SIZE'
            include 'SOLN'
            include 'MASS'
         ...
         end subroutine compute_frc_div_ifaxis
         ...
         subroutine pressure_matvec_2Dh_ifaxis(ap, wp, h1, h2, h2inv, alphaR2, intype)
            implicit none
            real(dp), dimension(lx2,ly2,lz2,lelv), intent(out) :: Ap
            real(dp), dimension(lx2,ly2,lz2,lelv), intent(in)  :: wp
            real(dp), dimension(lx1,ly1,lz1,lelv), intent(in)  :: h1, h2, h2inv, alphaR2
            ...
            call col2 (wdivm1, alphaR2, ntot1)            ! alpha^2/R^2 field == h2z_shift
         ...
         end subroutine pressure_matvec_2Dh_ifaxis
  ...
  9999      continue
         ...
         subroutine solve_pressure_2Dh_ifaxis(res,h1,h2,h2inv,alphaR2,alpha,intype,iter,ebar,info_str)
            implicit none
            include 'GMRES'
            real :: divex
            common  /ctolpr/ divex
            ...
            real, dimension(lx1,ly1,lz1,lelv), intent(in)    :: h1, h2, h2inv, alphaR2
            ...
            do while(iconv.eq.0.and.iter.lt.gmres_imax)
               if(iter.eq.0) then
                  ...
                  call pressure_matvec_2Dh_ifaxis(w_gmres,x_gmres,h1,h2,h2inv,alphaR2,intype)
               ...
               do j=1,m
                  ...
                  call pressure_matvec_2Dh_ifaxis(w_gmres,z_gmres(1,j),h1,h2,h2inv,alphaR2,intype)
  ...
  9000      continue
            ...
            if (present(info_str)) then
               if (nio.eq.0) write(6,9998) istep,'  U-PRES gmres  alpha= ', alpha,info_str, &
  ...
  9999      format(i11,a,I6,1p5e13.4)
         end subroutine solve_pressure_2Dh_ifaxis
